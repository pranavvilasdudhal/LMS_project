...
@section('admincontent')
    <div class="card shadow p-4">
        ...
        <form action="{{ route('enrolments.update', $enrolment->enrollment_id) }}" method="POST">
            ...
            <div class="form-group mb-3">
                ...
                <select name="student_id" class="form-control" required>
                    @foreach ($students as $stu)
                        <option value="{{ $stu->student_id }}" @if ($stu->student_id == $enrolment->student_id) selected @endif>
                            {{ $stu->user->name ?? 'N/A' }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group mb-3">
                ...
                <select name="course_id" id="courseSelect" class="form-control" required>
                    ...
                    @foreach ($courses as $course)
                        <option value="{{ $course->course_id }}" data-mrp="{{ $course->mrp }}"
                            data-sell="{{ $course->sell_price }}" @if ($course->course_id == $enrolment->course_id) selected @endif>
                            {{ $course->course_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group mb-3">
                <label>MRP</label>
                <input type="number" name="mrp" id="mrp" class="form-control" value="{{ $enrolment->mrp }}"
                    required readonly>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <select name="status" class="form-control">
                    ...
                    <option value="processing" {{ $enrolment->status == 'processing' ? 'selected' : '' }}>Processing
                    </option>
                    <option value="purchased" {{ $enrolment->status == 'purchased' ? 'selected' : '' }}>Purchased</option>
                </select>
            </div>
